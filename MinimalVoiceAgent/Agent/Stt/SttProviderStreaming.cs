...
{
    public static async Task<SpeechToTextConfig> LoadSttSettingsAsync(string filePath = "sttsettings.json")
    {
        if (!File.Exists(filePath))
        {
            Log.Error("STT settings file not found: {FilePath}", filePath);
            throw new FileNotFoundException($"STT settings file not found.", filePath);
        }
        ...
        try
        {
            ...
            var sttConfig = JsonSerializer.Deserialize<SpeechToTextConfig>(jsonContent, options)
                ?? throw new JsonException("Failed to deserialize STT settings JSON.");

            // Basic validation (expand as needed)
            if (string.IsNullOrEmpty(sttConfig.SttModelUrl))
            {
                throw new InvalidOperationException("SttModelUrl is required in STT settings.");
            }

            Log.Information("STT settings loaded successfully from {FilePath}", filePath);
            return sttConfig;
        }
        catch (JsonException ex)
        {
            Log.Error(ex, "Error deserializing STT settings from {FilePath}", filePath);
            throw;
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Unexpected error loading STT settings from {FilePath}", filePath);
            throw;
        }
    }
